...
def _get_recent_filings(cik: str, supplier_name: str) -> list[dict]:
    try:
        ...
        results = []
        for form, date, acc, doc in zip(forms, dates, accessions, descriptions, strict=False):
...
def crawl_edgar(redis_store) -> list[dict]:
    ...
    log.info(
        f"EDGAR crawl done — {len(new_items)} new filings from {len(public_suppliers)} suppliers"
    )
    return new_items
