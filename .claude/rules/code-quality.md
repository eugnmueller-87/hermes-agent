...
- Files/modules/functions: `snake_case`. Classes: `PascalCase`. Constants: `SCREAMING_SNAKE`.
- Booleans still `is_`/`has_`/`should_`/`can_`. No `l`, `I`, `O` single-char names.
- One public class/concern per module; helpers below the public API in call order.
